...
int main(void)
{
  ...
  /* USER CODE END 2 */
	...
	//RCC->AHBENR |= (1 << 17); // Enable peripheral clock for port A (17th bit is set)
	...
	GPIOA->OSPEEDR &= ~((1 << 0) | (1 << 1)); // Set speed to Low OSPEEDR0
	...
	EXTI->IMR |= (1 << 0); // Unmasked/Enabled
	EXTI->RTSR |= (1 << 0); // Rising-trigger enabled
	
	// Setup SYSCFG
	RCC->APB2ENR |= (1 << 0); // Substep: enable RCC to SYSCFG
	SYSCFG->EXTICR[0] &= ~((1 << 3) | (1 << 2) | (1 << 1) | (1 << 0)); // Route EXTI0 to PA0
...
{
	// Arbitrary delay (for checkoff 2)
	GPIOC->ODR ^= (1 << 8) | (1 << 9);
